...
wxPanel *CreateUserCreatedPage(wxBookCtrlBase *parent) {
    ...
    (void)new wxButton(panel, wxID_ANY, "Button", wxPoint(10, 10),
                       wxDefaultSize);
...
wxPanel *CreateRadioButtonsPage(wxBookCtrlBase *parent) {
    ...
    wxRadioBox *radiobox1 =
        new wxRadioBox(panel, wxID_ANY, "Choose one", wxDefaultPosition,
                       wxDefaultSize, 5, animals, 2, wxRA_SPECIFY_ROWS);

    wxString computers[] = {"Amiga", "Commodore 64", "PET", "Another"};

    wxRadioBox *radiobox2 = new wxRadioBox(
        panel, wxID_ANY, "Choose your favourite", wxDefaultPosition,
        wxDefaultSize, 4, computers, 0, wxRA_SPECIFY_COLS);
...
wxPanel *CreateVetoPage(wxBookCtrlBase *parent) {
    ...
    s->Add(
        new wxStaticText(panel, wxID_ANY, "This page intentionally left blank"),
...
wxWindow *CreateFullPageText(wxBookCtrlBase *parent) {
    wxTextCtrl *text =
        new wxTextCtrl(parent, wxID_ANY, "Full page text", wxDefaultPosition,
...
void CreateInitialPages(wxBookCtrlBase *parent) {
    ...
    parent->InsertPage(0, page, I_WAS_INSERTED_PAGE_NAME, false,
                       GetIconIndex(parent));
...
wxBEGIN_EVENT_TABLE(MyFrame, wxFrame)
    ...
    // Operations menu
    EVT_MENU(ID_ADD_PAGE, MyFrame::OnAddPage) EVT_MENU(
        ID_ADD_PAGE_NO_SELECT,
        MyFrame::
            OnAddPageNoSelect) EVT_MENU(ID_INSERT_PAGE,
                                        MyFrame::
                                            OnInsertPage) EVT_MENU(ID_DELETE_CUR_PAGE,
                                                                   MyFrame::
                                                                       OnDeleteCurPage)
        EVT_MENU(ID_DELETE_LAST_PAGE, MyFrame::OnDeleteLastPage) EVT_MENU(
            ID_NEXT_PAGE,
            MyFrame::OnNextPage) EVT_MENU(ID_CHANGE_SELECTION,
                                          MyFrame::OnChangeSelection)
            EVT_MENU(ID_SET_SELECTION, MyFrame::OnSetSelection) EVT_MENU(
                ID_GET_PAGE_SIZE,
                MyFrame::OnGetPageSize) EVT_MENU(ID_SET_PAGE_SIZE,
...
// Book controls
                        ...
                        EVT_NOTEBOOK_PAGE_CHANGED(
                            wxID_ANY,
                            MyFrame::
                                OnNotebook) EVT_NOTEBOOK_PAGE_CHANGING(wxID_ANY,
                                                                       MyFrame::
                                                                           OnNotebook)
#endif
#if wxUSE_LISTBOOK
                            EVT_LISTBOOK_PAGE_CHANGED(
                                wxID_ANY,
                                MyFrame::
                                    OnListbook) EVT_LISTBOOK_PAGE_CHANGING(wxID_ANY,
                                                                           MyFrame::
                                                                               OnListbook)
#endif
#if wxUSE_CHOICEBOOK
                                EVT_CHOICEBOOK_PAGE_CHANGED(
                                    wxID_ANY, MyFrame::OnChoicebook)
                                    EVT_CHOICEBOOK_PAGE_CHANGING(
                                        wxID_ANY, MyFrame::OnChoicebook)
#endif
#if wxUSE_TREEBOOK
                                        EVT_TREEBOOK_PAGE_CHANGED(
                                            wxID_ANY, MyFrame::OnTreebook)
                                            EVT_TREEBOOK_PAGE_CHANGING(
                                                wxID_ANY, MyFrame::OnTreebook)

                                                EVT_MENU(ID_ADD_SUB_PAGE,
    ...
    EVT_IDLE(MyFrame::OnIdle) wxEND_EVENT_TABLE()
    ...
    menuStyle->AppendCheckItem(ID_HORZ_LAYOUT,
                               "&Horizontal layout (wxToolbook)");
#endif

    wxMenu *menuPageOperations = new wxMenu;
    menuPageOperations->Append(ID_ADD_PAGE, "&Add page\tAlt-A");
    menuPageOperations->Append(ID_ADD_PAGE_NO_SELECT,
                               "&Add page (don't select)\tAlt-B");
    menuPageOperations->Append(ID_INSERT_PAGE, "&Insert page\tAlt-I");
    menuPageOperations->Append(ID_DELETE_CUR_PAGE,
                               "&Delete current page\tAlt-D");
    ...
    menuPageOperations->Append(ID_ADD_PAGE_BEFORE,
                               "Insert page &before\tAlt-B");
    menuPageOperations->Append(ID_ADD_SUB_PAGE, "Add s&ub page\tAlt-U");
#endif
    menuPageOperations->AppendSeparator();
    menuPageOperations->Append(ID_CHANGE_SELECTION,
                               "&Change selection to 0\tCtrl-0");
    menuPageOperations->Append(ID_SET_SELECTION,
                               "&Set selection to 0\tShift-Ctrl-0");
    menuPageOperations->AppendSeparator();
    ...
    menuFile->Append(wxID_ANY, "&Orientation", menuOrient,
    ...
    m_images.push_back(wxArtProvider::GetBitmapBundle(wxART_INFORMATION,
                                                      wxART_OTHER, imageSize));
    m_images.push_back(
        wxArtProvider::GetBitmapBundle(wxART_QUESTION, wxART_OTHER, imageSize));
    m_images.push_back(
        wxArtProvider::GetBitmapBundle(wxART_WARNING, wxART_OTHER, imageSize));
    m_images.push_back(
        wxArtProvider::GetBitmapBundle(wxART_ERROR, wxART_OTHER, imageSize));

    m_panel = new wxPanel(this);

#if USE_LOG
    m_text = new wxTextCtrl(m_panel, wxID_ANY, wxEmptyString, wxDefaultPosition,
...
#define CASE_SIMPLEBOOK(x)                                                     \
    case Type_Simplebook:                                                      \
        x;                                                                     \
        break;
...
void MyFrame::RecreateBook() {
    ...
    DISPATCH_ON_TYPE(
        m_bookCtrl = new, wxNotebook, wxListbook, wxChoicebook, wxTreebook,
        wxToolbook, wxAuiNotebook, wxSimplebook,
...
void MyFrame::OnHitTest(wxCommandEvent &WXUNUSED(event)) {
    ...
    AddFlagStrIfFlagPresent(flagsStr, flags, wxBK_HITTEST_NOWHERE,
                            "wxBK_HITTEST_NOWHERE");
    AddFlagStrIfFlagPresent(flagsStr, flags, wxBK_HITTEST_ONICON,
                            "wxBK_HITTEST_ONICON");
    AddFlagStrIfFlagPresent(flagsStr, flags, wxBK_HITTEST_ONLABEL,
                            "wxBK_HITTEST_ONLABEL");
    AddFlagStrIfFlagPresent(flagsStr, flags, wxBK_HITTEST_ONITEM,
                            "wxBK_HITTEST_ONITEM");
    AddFlagStrIfFlagPresent(flagsStr, flags, wxBK_HITTEST_ONPAGE,
                            "wxBK_HITTEST_ONPAGE");

    wxLogMessage("HitTest at (%d,%d): %d: %s", pt.x, pt.y, pagePos, flagsStr);
}
...
void MyFrame::OnStyle(wxCommandEvent &event) {
    ...
    switch (event.GetId()) {
    case ID_FIXEDWIDTH:
        m_fixedWidth = checked;
        break;
    case ID_MULTI:
        m_multi = checked;
        break;
    case ID_NOPAGETHEME:
        m_noPageTheme = checked;
        break;
    case ID_BUTTONBAR:
        m_buttonBar = checked;
        break;
    case ID_HORZ_LAYOUT:
        m_horzLayout = checked;
        break;
    default:
...
void MyFrame::OnExit(wxCommandEvent &WXUNUSED(event)) { Close(); }
...
void MyFrame::OnAddPage(wxCommandEvent &WXUNUSED(event)) {
    ...
    if (currBook) {
        ...
        currBook->AddPage(CreateNewPage(),
                          wxString::Format(ADDED_PAGE_NAME "%u", ++s_pageAdded),
...
void MyFrame::OnAddSubPage(wxCommandEvent &WXUNUSED(event)) {
    ...
    if (currBook) {
        ...
        currBook->InsertSubPage(
            selPos, CreateNewPage(),
            wxString::Format(ADDED_SUB_PAGE_NAME "%u", ++s_subPageAdded), true,
...
void MyFrame::OnAddPageBefore(wxCommandEvent &WXUNUSED(event)) {
    ...
    if (currBook) {
        ...
        currBook->InsertPage(
            selPos, CreateNewPage(),
            wxString::Format(ADDED_PAGE_NAME_BEFORE "%u", ++s_subPageAdded),
            true, GetIconIndex(currBook));
    }
}
#endif // wxUSE_TREEBOOK
...
void MyFrame::OnInsertPage(wxCommandEvent &WXUNUSED(event)) {
    ...
    if (currBook) {
        ...
        currBook->InsertPage(
            0, panel, wxString::Format(INSERTED_PAGE_NAME "%u", ++s_pageIns),
            false, GetIconIndex(currBook));
...
void MyFrame::OnGetPageSize(wxCommandEvent &WXUNUSED(event)) {
    ...
    wxLogMessage("Page size is (%d, %d), book size (%d, %d)", sizePage.x,
                 sizePage.y, sizeBook.x, sizeBook.y);
}
...
void MyFrame::OnSetPageSize(wxCommandEvent &WXUNUSED(event)) {
    ...
    wxLogMessage("Page size set to (%d, %d)", sizePage.x, sizePage.y);
}
...
void MyFrame::OnIdle(wxIdleEvent &WXUNUSED(event)) {
    ...
    if (nPages != s_nPages || nSel != s_nSel || s_currBook != currBook) {
        ...
        wxString title;
        title.Printf("Notebook and friends (%d pages, selection: %s)", nPages,
                     selection);
...
void MyFrame::OnBookCtrl(wxBookCtrlBaseEvent &event) {
...
#if wxUSE_NOTEBOOK
        {wxEVT_NOTEBOOK_PAGE_CHANGED, wxEVT_NOTEBOOK_PAGE_CHANGING,
         "wxNotebook"},
#endif // wxUSE_NOTEBOOK
#if wxUSE_LISTBOOK
        {wxEVT_LISTBOOK_PAGE_CHANGED, wxEVT_LISTBOOK_PAGE_CHANGING,
         "wxListbook"},
#endif // wxUSE_LISTBOOK
#if wxUSE_CHOICEBOOK
        {wxEVT_CHOICEBOOK_PAGE_CHANGED, wxEVT_CHOICEBOOK_PAGE_CHANGING,
         "wxChoicebook"},
#endif // wxUSE_CHOICEBOOK
#if wxUSE_TREEBOOK
        {wxEVT_TREEBOOK_PAGE_CHANGED, wxEVT_TREEBOOK_PAGE_CHANGING,
         "wxTreebook"},
#endif // wxUSE_TREEBOOK
#if wxUSE_TOOLBOOK
...
#if wxUSE_AUI
        {wxEVT_AUINOTEBOOK_PAGE_CHANGED, wxEVT_AUINOTEBOOK_PAGE_CHANGING,
         "wxAuiNotebook"},
#endif // wxUSE_AUI
    };
    ...
    //     wxRTTI
    const wxBookCtrlBase *const book =
        static_cast<wxBookCtrlBase *>(event.GetEventObject());
    ...
    wxLogMessage("Event #%d: %s: %s (%d) new sel %d, old %d, current %d%s",
                 ++s_num, nameControl, nameEvent, eventType,
                 event.GetSelection(), event.GetOldSelection(),
                 book->GetSelection(), veto);
